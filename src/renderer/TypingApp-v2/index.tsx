  ...
  const handleKeyPress = useCallback(
    (key: string) => {
      ...
      if (key === currentChar) {
        ...
        const secondsPassed = (currentTime() - (startTime || 0)) / 1000;
        setWpm(((updatedOutgoingChars.length * 10) / secondsPassed).toFixed(2));
      } else {
        setWrongChar(true);
      }
      ...
      <Grid size={12} p={2}>
        ...
        {!isGameActive && minutes === 0 && seconds === 0 && (
          <Alert severity="info">
            ...
            <Typography variant="h6">Nom de mots : {wordCount}</Typography>
            <Typography variant="h6">Mot par minute : {wpm}</Typography>
            <Typography variant="h6">Précision : {accuracy}%</Typography>
          </Alert>
        )}
      </Grid>
